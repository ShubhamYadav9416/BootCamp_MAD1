...
def user_profile():
    if request.method == "GET":
        ...
        no_of_following = UserFollower.query.filter_by(followed_user_id = user_logined_id).count()
...
def user_post(post_id):
    if request.method == "GET":
        ...
        comments = PostComments.query.join(Posts,PostComments.post_id == Posts.post_id).join(User,
            PostComments.user_id == User.user_id).filter(
                PostComments.post_id == post_id).add_columns(
                    User.user_name, User.user_mail, PostComments.comment).all()
        print(comments)
        return render_template("user_post.html",post=post,show_dlt_edit_buttons=show_dlt_edit_buttons,comments=comments)
...
def other_user(others_user_id):
    if request.method == "GET":
        ...
        following_details = UserFollower.query.filter_by(follower_user_id = others_user_id).filter_by(followed_user_id = user_logined_id).first()
        ...
        no_of_following = UserFollower.query.filter_by(follower_user_id = others_user_id).count()
        no_of_follower = UserFollower.query.filter_by(followed_user_id = others_user_id).count()
...
def follow(others_user_id):
    user_logined_id = current_user.user_id
    follow = UserFollower(follower_user_id = others_user_id, followed_user_id = user_logined_id)
...
def unfollow(others_user_id):
    user_logined_id  = current_user.user_id
    unfollow = UserFollower.query.filter_by(follower_user_id = others_user_id).filter_by(followed_user_id = user_logined_id).first()
...
def comment(post_id):
    if request.method == "POST":
        ...
        new_comment = PostComments(user_id = current_user.user_id, post_id=post_id,comment=comment )
